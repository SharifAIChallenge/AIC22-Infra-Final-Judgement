...
def __judge(first_team_name, second_team_name) -> int:
    ...
    try:
        ...
        output = check_output(match_runcommand[:3] + [f"--first-team-name={first_team_name.replace(' ', '_')}" + f" --second-team-name={second_team_name.replace(' ', '_')}"] + match_runcommand[3:], stderr=STDOUT, timeout=match_timeout)
...
def judge(players, map_id, game_id, first_team_name, second_team_name) -> [Event]:
    ...
    try:
        ...
        with open(match_record_path, 'r') as file:
            stats = file.read()
    except:
