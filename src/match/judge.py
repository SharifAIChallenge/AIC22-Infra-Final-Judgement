...
def __judge(first_team_name, second_team_name) -> int:
    ...
    try:
        ...
        output = check_output(match_runcommand + [f"--first-team-name={first_team_name.replace(' ', '_')}", f"--second-team-name={second_team_name.replace(' ', '_')}"], stderr=STDOUT, timeout=match_timeout)
